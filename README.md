...
$$\boxed{\begin{cases}
\partial_t \phi_t + \dfrac{1}{2} |\nabla \phi_t|^2 + \beta_{t}(x).\nabla \phi + \gamma \Delta \phi_t = 0, \\
...
$$\begin{aligned} g^{(k+1)}(x) &= \log \nu(x) - \log\!\left( \mathcal{Q}_1\!\left( e^{f^{(k)}} \right)(x) \right), \\ f^{(k+1)}(x) &= \log \mu(x) - \log\!\left( \mathcal{Q}_1\!\left( e^{g^{(k+1)}} \right)(x) \right). \end{aligned}$$
...
$$\boxed{K_t\eta_t = \dot m_t - \gamma \ell_t;}\qquad
\begin{aligned}
(\kappa_t)_{ij} &= \mathbb{E}_{\rho_t}[\nabla\varphi_i\cdot\nabla\varphi_j],\\
(m_t)_i &= \mathbb{E}_{\rho_t}[\varphi_i],\\
(\ell_t)_i &= \mathbb{E}_{\rho_t}[\Delta\varphi_i].
\end{aligned}$$

**Practical recipe**

1. At each snapshot time $t_k$​, from N samples: assemble $\hat{K}_{tk}, \hat{m}_{t_k}, \hat{\ell}_{t_k}, (\Delta \varphi$​ via Hutchinson if second derivatives are expensive.)
2. Smooth $k \to \hat{m}_{t_{k}}​​$ (cubic spline / local polynomial / a small parametric fit in $t$) and differentiate $\to \dot m_{t_k}​​$.
3. Solve $(\hat K_{t_k}+\lambda I)\eta_{t_k} = \dot m_{t_k}-\gamma\hat\ell_{t_k}​​$. Ridge is essential: $K_{t}​$ is badly conditioned for scattering features.
4. Interpolate $\eta_t$ in t, set $\beta_t = \nabla\varphi^\top\eta_t$​, feed into your drifted IPFP.
...
- **Prescribed path.** The square is carried by a time-dependent affine map
  $\Phi_t(y) = \lambda(t) y + b(t)$: a sine-arc translation plus a breathing size,
  pinned so $\Phi_0 = \mathrm{id}$ and $\Phi_1$ sends $\mu$ to $\nu$.
- **Exact, curl-free drift.** Being affine in space, its velocity
  $\beta_t(x) = (\lambda'/\lambda)(x - b_t) + b'_t$ is the gradient of a quadratic.
- **Comparison.** Runs with no drift, a constant drift, and the exact drift (at
  decreasing $\gamma$) are compared against the noiseless reference path via the L1
  distance and a debiased entropic Wasserstein-2 distance. As $\gamma \to 0$, only the
  exact-drift runs converge to the curved reference; the driftless bridge instead
  converges to the straight-line displacement interpolation.
